...
public class CadastrarStakeholders implements InterfaceCommand {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)	throws Exception{
		try {
			...
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM stakeholders WHERE codigoprojeto = '"+request.getParameter("codigoprojeto")+"'");// pega a conexão e o Statement
			...
			while(rs.next()){
				...
				if(codigoprojeto >= 1){
					...
					out.println("alert('Já existe um registro com o código do projeto ("+stakeholders.getCodigoprojeto()+"), informado!');");  
					out.println("</script>");
