...
public class CadastrarOrganograma implements InterfaceCommand {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)	throws Exception{
		try {
			...
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM organograma WHERE projetoinicio = '"+request.getParameter("projetoinicio")+"'");// pega a conexão e o Statement
			...
			while(rs.next()){
				...
				if(codigoprojeto >= 1){
					...
					out.println("alert('Já existe um registro com o código do projeto ("+organograma.getProjetoinicio().getProjeto()+"), informado!');");  
					out.println("</script>");
