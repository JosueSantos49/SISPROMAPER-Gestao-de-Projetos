...
public class CadastrarRfq implements InterfaceCommand {

	private InterfaceRfqDAO rfqDAO;
	public CadastrarRfq(InterfaceRfqDAO rfqDAO) {
		super();
		this.rfqDAO = rfqDAO;
	}
	@Override
	public String execute(HttpServletRequest request, HttpServletResponse response)	throws Exception{
		try {
			...
			Date datalimite =  new SimpleDateFormat("dd/MM/yyyy").parse(datalimite_1);
			...
			rfq.setPuitem1(Double.parseDouble(request.getParameter("puitem1")));
			rfq.setPuitem2(Double.parseDouble(request.getParameter("puitem2")));
			rfq.setPuitem3(Double.parseDouble(request.getParameter("puitem3")));
			rfq.setPuitem4(Double.parseDouble(request.getParameter("puitem4")));
			rfq.setPuitem5(Double.parseDouble(request.getParameter("puitem5")));
			rfq.setPuitem6(Double.parseDouble(request.getParameter("puitem6")));
			rfq.setPuitem7(Double.parseDouble(request.getParameter("puitem7")));
			rfq.setPuitem8(Double.parseDouble(request.getParameter("puitem8")));
			rfq.setPuitem9(Double.parseDouble(request.getParameter("puitem9")));
			rfq.setPuitem10(Double.parseDouble(request.getParameter("puitem10")));
			rfq.setPuitem11(Double.parseDouble(request.getParameter("puitem11")));
			rfq.setPuitem12(Double.parseDouble(request.getParameter("puitem12")));
			rfq.setPuitem13(Double.parseDouble(request.getParameter("puitem13")));
			rfq.setPuitem14(Double.parseDouble(request.getParameter("puitem14")));
			rfq.setPuitem15(Double.parseDouble(request.getParameter("puitem15")));
			rfq.setPuitem16(Double.parseDouble(request.getParameter("puitem16")));
			rfq.setPuitem17(Double.parseDouble(request.getParameter("puitem17")));
			rfq.setPuitem18(Double.parseDouble(request.getParameter("puitem18")));
			rfq.setPuitem19(Double.parseDouble(request.getParameter("puitem19")));
			rfq.setPuitem20(Double.parseDouble(request.getParameter("puitem20")));
			...
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM rfq WHERE numerorfq = '"+request.getParameter("numerorfq")+"'");// pega a conexão e o Statement
			ResultSet rs = stmt.executeQuery();// executa um select
			
			while(rs.next()){
				
				Integer numerorfq = rs.getInt("numerorfq");
				...
				if(numerorfq >= 1){
					...
					out.println("alert('Já existe um registro com o numero ("+rfq.getNumerorfq()+"), informado!');");  
					out.println("</script>");
					
					return "TkcsapcdController?cmd=editarRfq";//devolver p/ editar cadastro	
				}else{
					System.out.println("Aviso 1: O código não é igual!");
					rfqDAO.salvar(rfq);
					request.setAttribute("mensagem", "Pedido de Cotação: ("+rfq.getNumerorfq()+") cadastrado com sucesso!");
				}
					stmt.close();
					rs.close();
					con.close();
				...
				request.setAttribute("titulo", "Cadastro - Pedido de Cotação");
				return "TkcsapcdController?cmd=consultarRfq";//devolver p/ editar cadastro		
