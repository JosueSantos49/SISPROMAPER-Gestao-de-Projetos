...
public class CadastrarOrcamentoProjeto implements InterfaceCommand {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)	throws Exception{
		try {
			...
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM orcamentoprojeto WHERE codigoprojeto = '"+request.getParameter("codigoprojeto")+"'");// pega a conexão e o Statement
			...
			while(rs.next()){
				...
				if(codigoprojeto >= 1){
					...
					out.println("alert('Já existe um registro com o código do projeto ("+orcamentoProjeto.getCodigoprojeto()+"), informada!');");  
					out.println("</script>");
					...
					request.setAttribute("mensagem", "Orçamento do projeto: ("+orcamentoProjeto.getCodigoprojeto()+") cadastrado com sucesso!");
				}
				stmt.close();
				rs.close();
				con.close();
				...
				request.setAttribute("titulo", "Cadastro - Orçamento do projeto");
				return "TkcsapcdController?cmd=editarOrcamentoProjeto";//devolver p/ editar cadastro		
	}
}
