...
<%@page import="java.util.List,br.com.tkcsapcd.model.bean.SolicitacaoMudanca"%>
<%@page import="br.com.tkcsapcd.model.bean.SolicitacaoMudanca"%>
...
<title>Consultar Solicitação de Mudança</title>
...
<%@include file="menuperantecssadm.jsp"%>
	...
	<div id="conteudo">
		...
		<div align="justify">			
			...
			<div align="center">
				<p>
					<font color="red">Consultar Solicitação de Mudança</font>
				</p>
			</div>
			<br>
			...
			<div align="left">
				...
				<form id="formConsProjeto" name="formConsProjeto" method="post"
					action="pesquisar_solicitacao_mudanca.jsp"
					...
					<strong>Relatório..:</strong> <input name="consProjeto" type="text"
						...
						required="true" Placeholder="Código da Versão!"> <input
			...
			<div class="table-responsive">
				<table id="tabela" cellspacing="5" cellspacing="5" class="display">
					...
					<tbody>
							<%
								List<SolicitacaoMudanca> solicitacaoMudancas = (List<SolicitacaoMudanca>) request.getAttribute("solicitacaoMudancas");
								for (SolicitacaoMudanca solicitacaoMudanca : solicitacaoMudancas) {
							%>
						<tr>							
							<td><%=solicitacaoMudanca.getProjetoinicio().getCodigo()%></td>
							<td><%=solicitacaoMudanca.getProjetoinicio().getProjeto() %></td>	
							<td><%=solicitacaoMudanca.getProjetoinicio().getCodigoprojeto()%></td>						
							<td><%=solicitacaoMudanca.getSolicitacaomudanca()%></td>
							<td><%=solicitacaoMudanca.getVersao()%></td>
							<td><%=solicitacaoMudanca.getData()%></td>
							<td><%=solicitacaoMudanca.getAutor()%></td>
							<td><%=solicitacaoMudanca.getNotarevisao()%></td>
							<td><a class="link_atualizar"
								href='TkcsapcdController?cmd=editarSolicitacaoMudanca&codigo=<%=solicitacaoMudanca.getCodigo()%>'><img
									alt="" title="Atualizar" height="20" width="27"
									src="img/alterar.gif"></a></td>
							<td><a class="link_delete"
								href='TkcsapcdController?cmd=excluirSolicitacaoMudanca&codigo=<%=solicitacaoMudanca.getCodigo()%>'><img
