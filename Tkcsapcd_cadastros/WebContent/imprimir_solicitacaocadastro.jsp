...
<table border="0" width="1100" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO GERAL SOLICITAÇÃO CADASTRO</h3></font>
								<div align="right">
...
<div class="table-responsive">
   <table class="table" id="tableID"> 
    <tr class="info">
      <td><div align="center" class="style6"><a href="imprimir_solicitacaocadastro.jsp?ordenacao=crescente">CÓDIGO</a></div></td>
      <td><div align="center" class="style6"><a href="imprimir_solicitacaocadastro.jsp?ordenacao=alfabetica">NOME</a></div></td>
      ...
      <td><div align="center" class="style5"><strong>FUNÇÃO</strong></div></td>
	...
	try
	{
		...
		//Processo Paginação
				...
				String sql1 = "select * from solicitacaocadastro LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from solicitacaocadastro order by nome");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from solicitacaocadastro");
			}		
	...
	<%
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_solicitacaocadastro.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_solicitacaocadastro.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_solicitacaocadastro.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_solicitacaocadastro.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <table width="400" border="0">
    <tr>
      <td><div align="center"> <span class="style1 style2 style5">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_solicitacaocadastro.jsp')"/>
       <input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_solicitacaocadastro.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
      </span></div></td>
    </tr>
  </table>
</div>
