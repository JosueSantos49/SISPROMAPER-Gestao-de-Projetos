...
<title>Imprimir Seleção Fonte (Pré-qualificação)</title>
	...
	<table border="0" width="900" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO GERAL SELEÇÃO FONTE (Pré-qualificação)</h3></font>	
								<div align="right">
...
<div align="center">
  <table width="1100" border="0">
    <tr bgcolor="#CCCCCC">
    	<td width="90"><div align="" class="style6"><a href="imprimir_criterio_pre_qualificacao.jsp?ordenacao=crescente">CÓDIGO PROJETO</a></div></td>
      	<td width="90"><div align="" class="style6"><a href="imprimir_criterio_pre_qualificacao.jsp?ordenacao=alfabetica">NOME DO PROJETO</a></div></td>
      	<td width="90"><div align="" class="style5"><strong>RFP</strong></div></td>  
      	<td width="90"><div align="" class="style5"><strong>GERENTE DE PROJETO</strong></div></td>   
    </tr>
    <%
    Connection con = null;
	...
	try
	{
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from criterioprequalificacao where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from criterioprequalificacao order by codigoprojeto");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from criterioprequalificacao");
			}		
		else if(request.getParameter("ordenacao").equals("alfabetica"))
			{
				rs = st.executeQuery("select * from criterioprequalificacao order by projeto");
			}
	...
	%>
        <tr bgcolor="#FFFFFF">
         	<td><div align="left"><%=rs.getString("codigoprojeto")%></div></td>
          	...
          	<td><div align="left"><%=rs.getString("rfp")%></div></td>
          	<td><div align="left"><%=rs.getString("gp")%></div></td>
        </tr>
	<%
			}
		rs.close();
  ...
  <br>
	 <div align="center">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_criterio_pre_qualificacao.jsp')"/>
       	<input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_criterio_pre_qualificacao.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
 	</div>
</body>
</html>
