...
<title>Imprimir Decisões de Comprar</title>
	...
	<table border="0" width="900" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO GERAL DECISÕES DE COMPRAR</h3></font>	
								<div align="right">
...
<div class="table-responsive">
   <table class="table" > 
    <tr class="info">
    	<td width="90"><div align="" class="style6"><a href="imprimir_decisoes-comprar.jsp?ordenacao=crescente">CÓDIGO EAP</a></div></td>
      	<td width="90"><div align="" class="style6"><a href="imprimir_decisoes-comprar.jsp?ordenacao=alfabetica">IMPORTÂNCIA</a></div></td>
      	<td width="90"><div align="" class="style5"><strong>URGÊNCIA</strong></div></td>  
      	<td width="90"><div align="" class="style5"><strong>IMPACTO</strong></div></td>
      	<td width="90"><div align="" class="style5"><strong>QUANTIDADE</strong></div></td> 
      	<td width="90"><div align="" class="style5"><strong>CUSTO</strong></div></td>   
    </tr>
    <%
    Connection con = null;
	...
	try
	{
		...
		//Processo Paginação
				...
				String sql1 = "select * from decisoescomprar LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from decisoescomprar where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from decisoescomprar order by codigoeap");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from decisoescomprar");
			}		
		else if(request.getParameter("ordenacao").equals("alfabetica"))
			{
				rs = st.executeQuery("select * from decisoescomprar order by notarevisao");
			}
	...
	<tr class="default">
        ...
        <tr class="success">
         	<td><div align="left"><%=rs.getString("codigoeap")%></div></td>
          	<td><div align="left"><%=rs.getString("importancia")%></div></td>          
          	<td><div align="left"><%=rs.getString("urgencia")%></div></td>
          	<td><div align="left"><%=rs.getString("impacto")%></div></td>
          	<td><div align="left"><%=rs.getString("quantidade")%></div></td>
          	<td><div align="left"><%=rs.getString("custo")%></div></td>
        </tr>
	<%
			}
		//Processo Paginação
	    String sqlConta = "select count(*) AS contaRegistros from decisoescomprar";
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_decisoes-comprar.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_decisoes-comprar.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_decisoes-comprar.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_decisoes-comprar.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <br>
	 <div align="center">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_decisoes-comprar.jsp')"/>
       	<input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_decisoes-comprar.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
 	</div>
</body>
</html>
