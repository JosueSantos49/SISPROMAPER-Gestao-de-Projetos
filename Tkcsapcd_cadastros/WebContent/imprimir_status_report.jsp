	...
	<table border="0" width="900" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO GERAL STATUS REPORT</h3></font>	
								<div align="right">
...
<div class="table-responsive">
   <table class="table" > 
    <tr class="info">
    	<td ><div align="" class="style6"><a href="imprimir_status_report.jsp?ordenacao=crescente">CÓDIGO</a></div></td>
      	<td ><div align="" class="style6"><a href="imprimir_status_report.jsp?ordenacao=alfabetica">PROJETO</a></div></td>
      	<td ><div align="" class="style5"><strong>STATUS REPORT</strong></div></td>  
      	<td ><div align="" class="style5"><strong>PER. INÍCIO</strong></div></td>
      	<td ><div align="" class="style5"><strong>PER. FIM</strong></div></td> 
      	<td ><div align="" class="style5"><strong>INDICADOR POR ÁREA</strong></div></td>
	...
	try
	{
		...
		//Processo Paginação
				...
				String sql1 = "select * from statusreport LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from statusreport where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from statusreport order by codigoprojeto");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from statusreport");
			}		
		else if(request.getParameter("ordenacao").equals("alfabetica"))
			{
				rs = st.executeQuery("select * from statusreport order by projeto");
			}
	...
	<tr class="default">
        ...
        <tr class="success">
         	<td><div align="left"><%=rs.getString("codigoprojeto")%></div></td>
          	...
          	<td><div align="left"><%=rs.getString("statusreport")%></div></td>
          	<td><div align="left"><%=rs.getString("periodoinicio")%></div></td>
          	<td><div align="left"><%=rs.getString("periodofim")%></div></td>
          	<td><div align="left"><%=rs.getString("indicador")%></div></td>
          	<td><div align="left"><%=rs.getString("status")%></div></td>
        </tr>
	<%
			}
		//Processo Paginação
	    String sqlConta = "select count(*) AS contaRegistros from statusreport";
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_status_report.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_status_report.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_status_report.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_status_report.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <br>
	 <div align="center">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_status_report.jsp')"/>
       	<input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_status_report.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
 	</div>
</body>
</html>
