...
<title>Relatório Solicitação Cadastro</title>
...
<table border="0" width="1300" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO TERMO DE ENCERRAMENTO DO PROJETO</h3></font>
								<div align="right">
...
<div class="table-responsive">
   <table class="table" id="tableID"> 
    <tr class="info">
      <td><div align="" class="style5"><a href="imprimir_termoencerramentoprojeto.jsp?ordenacao=crescente"><strong>CÓDIGO</a></strong></div></td>
      <td><div align="" class="style6"><a href="imprimir_termoencerramentoprojeto.jsp?ordenacao=alfabetica">PROJETO</a></div></td>      
      <td><div align="" class="style5"><strong>DATA DOCUMENTO</strong></div></td>
      <td><div align="" class="style5"><strong>COORDENADOR</strong></div></td>
      <td><div align="" class="style5"><strong>TIPO PROJETO</strong></div></td>
      <td><div align="" class="style5"><strong>GESTOR</strong></div></td>
      <td><div align="" class="style5"><strong>PROGRAMA</strong></div></td> 
      <td><div align="" class="style5"><strong>STATUS DE CONCLUSÃO</strong></div></td>
      <td><div align="" class="style5"><strong>DATA DE ANÁLISE DE RESULTADO</strong></div></td>    
    </tr>
    <%
    Connection con = null;
	...
	try
	{
		...
		String sql1 = "select * from projetotermoencerramento LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from projetotermoencerramento where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from projetotermoencerramento order by codigo");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from projetotermoencerramento");
			}		
		else if(request.getParameter("ordenacao").equals("alfabetica"))
			{
				rs = st.executeQuery("select * from projetotermoencerramento order by dataanaliseresultado");
			}
	...
	<tr class="default">
        ...
        <tr class="success">
          <td><div align="left"><%=rs.getString("nomeprojeto")%></div></td>          
          <td><div align="left"><%=rs.getString("codigoprojeto")%></div></td>
          <td><div align="left"><%=rs.getString("datadocumento")%></div></td>
          <td><div align="left"><%=rs.getString("coordenador")%></div></td>
          <td><div align="left"><%=rs.getString("tipoprojeto")%></div></td>
          <td><div align="left"><%=rs.getString("gestor")%></div></td>
          <td><div align="left"><%=rs.getString("programa")%></div></td> 
          <td><div align="left"><%=rs.getString("statusconclusao")%></div></td>
          <td><div align="left"><%=rs.getString("dataanaliseresultado")%></div></td>         
        </tr>
	<%
			}
		
		//Processo Paginação
	    String sqlConta = "select count(*) AS contaRegistros from projetotermoencerramento";
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_termoencerramentoprojeto.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_termoencerramentoprojeto.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_termoencerramentoprojeto.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_termoencerramentoprojeto.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <table width="400" border="0">
    <tr>
      <td><div align="center"> <span class="style1 style2 style5">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_termoencerramentoprojeto.jsp')"/>
       <input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_termoencerramentoprojeto.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
      </span></div></td>
    </tr>
  </table>
</div>
