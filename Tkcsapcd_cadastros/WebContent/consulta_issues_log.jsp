...
<%@page import="br.com.tkcsapcd.model.bean.IssuesLog"%>
...
<title>Consultar ISSUES LOG (Registro das questões)</title>
...
<body onload="waitingDialog.show();setTimeout(function () {waitingDialog.hide();}, 2000); tamanhooriginal(); setFocus();">
	...
	<div id="conteudo">
		...
		<div align="justify">
			...
			<div align="center">
				<p>
					<font color="red">Consultar ISSUES LOG (Registro das questões)</font>
				</p>
			</div>
			<br>
			...
			<div align="left">
				...
				<form id="formConsProjeto" name="formConsProjeto" method="post"
					action="pesquisar_issues_log.jsp"
			...
			<div class="table-responsive">
				<table id="tabela" cellspacing="5" cellspacing="5" class="display">
					...
					<tbody>
							<%
								List<IssuesLog> issuesLogs = (List<IssuesLog>) request.getAttribute("issuesLogs");
								for (IssuesLog issuesLog : issuesLogs) {
							%>
						<tr>							
							<td><%=issuesLog.getCodigoprojeto()%></td>
							<td><%=issuesLog.getCodigoissueslog()%></td>
							<td><%=issuesLog.getProjeto()%></td>
							<td><%=issuesLog.getIssueslog()%></td>
							<td><%=issuesLog.getVersao()%></td>
							<td><%=issuesLog.getData()%></td>
							<td><%=issuesLog.getAutor()%></td>
							<td><%=issuesLog.getNotarevisao()%></td>
							<td><%=issuesLog.getUrgencia()%></td>
							<td><%=issuesLog.getImpacto()%></td>
							<td><%=issuesLog.getStatus()%></td>
							<td><a class="link_atualizar"
								href='TkcsapcdController?cmd=editarIssuesLog&codigo=<%=issuesLog.getCodigo()%>'><img
									alt="" title="Atualizar" height="20" width="27"
									src="img/alterar.gif"></a></td>
							<td><a class="link_delete"
								href='TkcsapcdController?cmd=excluirIssuesLog&codigo=<%=issuesLog.getCodigo()%>'><img
