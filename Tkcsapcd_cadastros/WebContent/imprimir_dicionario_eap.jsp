...
<title>Imprimir Dicionário da EAP</title>
	...
	<table border="0" width="900" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO GERAL DICIONÁRIO DA EAP</h3></font>	
								<div align="right">
...
<div align="center" class="table-responsive">
  <table width="1200" border="0" class="table">
    <tr class="info">
    	<td ><a href="imprimir_dicionario_eap.jsp?ordenacao=crescente">CÓDIGO</a></div></td>
      	<td ><a href="imprimir_dicionario_eap.jsp?ordenacao=alfabetica"><strong>DICIONÁRIO EAP</strong></a></div></td>  
      	<td ><strong>CÓD. EAP</strong></div></td>  
      	<td ><strong>VERSÃO</strong></div></td>
      	<td ><strong>DATA</strong></div></td> 
      	<td ><strong>AUTOR</strong></div></td>
      	<td ><strong>NOTA DE REVISÃO</strong></div></td>  
      	<td ><strong>CUSTO</strong></div></td>
	...
	try
	{
		...
		//Processo Paginação
				...
				String sql1 = "select * from dicionarioeap LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from dicionarioeap where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from dicionarioeap order by codigo");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from dicionarioeap");
			}		
		else if(request.getParameter("ordenacao").equals("alfabetica"))
			{
				rs = st.executeQuery("select * from dicionarioeap order by projeto");
			}
	...
	<tr class="default">
        ...
        <tr class="success">
         	<td><div align="left"><%=rs.getString("codigo")%></div></td>         
          	...
          	<td><div align="left"><%=rs.getString("custos")%></div></td>
          	<td><div align="left"><%=rs.getString("status")%></div></td>
        </tr>
	<%
			}
		//Processo Paginação
	    String sqlConta = "select count(*) AS contaRegistros from dicionarioeap";
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_dicionario_eap.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_dicionario_eap.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_dicionario_eap.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_dicionario_eap.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <br>
	 <div align="center">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_dicionario_eap.jsp')"/>
       	<input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_dicionario_eap.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
 	</div>
</body>
</html>
