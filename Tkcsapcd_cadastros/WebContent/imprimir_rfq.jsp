...
<title>Imprimir Pedido de Cotação</title>
	...
	<table border="0" width="900" align="center">
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO GERAL PEDIDO DE COTAÇÃO</h3></font>	
								<div align="right">
...
<div class="table-responsive">
   <table class="table" id="tableID"> 
    <tr class="info">
    	<td ><div  class="style6"><a href="imprimir_rfq.jsp?ordenacao=crescente">NÚMERO RFQ</a></div></td>
      	<td ><div  class="style6"><a href="imprimir_rfq.jsp?ordenacao=alfabetica">PRAZO ENTREGA</a></div></td>
      	<td ><div  class="style5"><strong>DATA EMISSÃO</strong></div></td>
      	<td ><div  class="style5"><strong>DATA LIMITE</strong></div></td>
      	<td ><div  class="style5"><strong>CONTATO COMERCIAL</strong></div></td>
      	<td ><div  class="style5"><strong>CONTATO TÉCNICO</strong></div></td> 
      	<td ><div  class="style5"><strong>EMPRESA</strong></div></td>   
      	<td ><div  class="style5"><strong>TEMPO GARANTIA</strong></div></td> 
      	<td ><div  class="style5"><strong>TERMO PAGAMENTO</strong></div></td>    
      	<td ><div  class="style5"><strong>VALOR TOTAL</strong></div></td> 
      	<td ><div  class="style5"><strong>FRETE</strong></div></td> 
    </tr>
    <%
    Connection con = null;
	...
	try{		
		...
		String sql1 = "select * from rfq LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from rfq where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from rfq order by numerorfq");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from rfq");
			}		
	...
	<tr class="default">
        ...
        <tr class="success">
         	<td><div align="left"><%=rs.getString("numerorfq")%></div></td>
         	<td><div align="left"><%=rs.getString("prazoentrega")%></div></td>
          	<td><div align="left"><%=rs.getString("data")%></div></td>          
          	<td><div align="left"><%=rs.getString("datalimite")%></div></td>
          	<td><div align="left"><%=rs.getString("contatocomercial")%></div></td>
          	<td><div align="left"><%=rs.getString("contatotecnico")%></div></td>
          	<td><div align="left"><%=rs.getString("empresa")%></div></td>     
          	<td><div align="left"><%=rs.getString("tempogarantia")%></div></td>  
          	<td><div align="left"><%=rs.getString("termopagamento")%></div></td> 
          	<td><div align="left"><%=rs.getString("total")%></div></td>
          	<td><div align="left"><%=rs.getString("frete")%></div></td>    	
        </tr>
		<%
			}
			
			//Processo Paginação
		    String sqlConta = "select count(*) AS contaRegistros from rfq";
			...
			if(Integer.parseInt(numPagina) > 1){
				...
				out.println("<li><a href=imprimir_rfq.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
				out.println("</ul>");
			}			
			for(int i=1;i<=totalPaginas;i++){
				if(i == Integer.parseInt(numPagina))
					...
					out.println("<a href=imprimir_rfq.jsp?numpagina="+i+"></a>");
					out.println("<ul class='pagination'>");
					out.println("<li><a href=imprimir_rfq.jsp?numpagina="+i+">"+ i +"</a></li>");
					out.println("</ul>");
			}											
			int proximaPag;
			if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
				...
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_rfq.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
				out.println("</ul>");
			}
  ...
  <br>
	 <div align="center">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_rfq.jsp')"/>
       	<input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_rfq.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
 	</div>
</body>
</html>
