...
<%@page import="br.com.tkcsapcd.model.bean.AvaliacaoDesempenhoEquipe"%>
...
<title>Consultar Avaliação do Desempenho da Equipe</title>
...
<body onload="waitingDialog.show();setTimeout(function () {waitingDialog.hide();}, 2000); tamanhooriginal(); setFocus();">
	...
	<div id="conteudo">
		...
		<div align="justify">
			...
			<div align="center">
				<p>
					<font color="red">Consultar Avaliação do Desempenho da Equipe</font>
				</p>
			</div>
			<br>
			...
			<div align="left">
				...
				<form id="formConsProjeto" name="formConsProjeto" method="post"
					action="pesquisar_avaliacao_desempenho_equipe.jsp"
				...
				<table id="tabela" cellspacing="5" cellspacing="5" class="display">
					<thead>
						<tr>
							...
							<th>Avaliação Desempenho Equipe</th>
							<th>Versão</th>
							<th>Data</th>
							<th>Autor</th>
					...
					<tbody>
							<%
								List<AvaliacaoDesempenhoEquipe> avaliacaoDesempenhoEquipes = (List<AvaliacaoDesempenhoEquipe>) request.getAttribute("avaliacaoDesempenhoEquipes");
								for (AvaliacaoDesempenhoEquipe avaliacaoDesempenhoEquipe : avaliacaoDesempenhoEquipes) {
							%>
						<tr>							
							<td><%=avaliacaoDesempenhoEquipe.getCodigoprojeto()%></td>
							<td><%=avaliacaoDesempenhoEquipe.getProjeto()%></td>
							<td><%=avaliacaoDesempenhoEquipe.getAvaliacaodesempenhoequipe()%></td>
							<td><%=avaliacaoDesempenhoEquipe.getVersao()%></td>
							<td><%=avaliacaoDesempenhoEquipe.getData()%></td>
							<td><%=avaliacaoDesempenhoEquipe.getAutor()%></td>
							<td><%=avaliacaoDesempenhoEquipe.getNotarevisao()%></td>							
							<td><%=avaliacaoDesempenhoEquipe.getEquipe()%></td>
							<td><%=avaliacaoDesempenhoEquipe.getPeriodoinicio()%></td>
							<td><%=avaliacaoDesempenhoEquipe.getPeriodofim()%></td>
							<td><%=avaliacaoDesempenhoEquipe.getIndicador()%></td>
							<td><%=avaliacaoDesempenhoEquipe.getStatus()%></td>
							<td><a class="link_atualizar"
								href='TkcsapcdController?cmd=editarAvaliacaoDesempenhoEquipe&codigo=<%=avaliacaoDesempenhoEquipe.getCodigo()%>'><img
									alt="" title="Atualizar" height="20" width="27"
									src="img/alterar.gif"></a></td>
							<td><a class="link_delete"
								href='TkcsapcdController?cmd=excluirAvaliacaoDesempenhoEquipe&codigo=<%=avaliacaoDesempenhoEquipe.getCodigo()%>'><img
