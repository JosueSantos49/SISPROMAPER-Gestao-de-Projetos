...
<title>Imprimir Ata de Reunião</title>
	...
	<table border="0" width="900" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO GERAL ATA DE REUNIÃO</h3></font>	
								<div align="right">
...
<div class="container">
  <table class="table">
    <tr class="danger">
    	<td ><div ><a href="imprimir_stakeholders.jsp?ordenacao=crescente">CÓDIGO</a></div></td>
      	<td ><div ><a href="imprimir_stakeholders.jsp?ordenacao=alfabetica">PROJETO</a></div></td>
      	<td ><div ><strong>ATA DE REUNIÃO</strong></div></td>
      	<td ><div ><strong>DATA</strong></div></td>
      	<td ><div ><strong>LOCAL</strong></div></td>  
      	<td ><div ><strong>PARTICIPANTE</strong></div></td>
      	<td ><div ><strong>APROVAÇÃO</strong></div></td>     
    </tr>
    <%!
    Connection con = null;
	...
	try
	{
				...
				String sql1 = "select * from atareuniao LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from atareuniao where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from atareuniao, projetoinicio WHERE projetoinicio = projetoinicio order by codigoprojeto");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from atareuniao, projetoinicio WHERE projetoinicio = projetoinicio");
			}		
		else if(request.getParameter("ordenacao").equals("alfabetica"))
			{
				rs = st.executeQuery("select * from atareuniao, projetoinicio WHERE projetoinicio = projetoinicio order by codigo");
			}
	...
	%>
        <tr class="success">
         	<td><div align="left"><%=rs.getString("codigoprojeto")%></div></td>
          	...
          	<td><div align="left"><%=rs.getString("ata")%></div></td>
          	<td><div align="left"><%=rs.getString("data")%></div></td>
          	<td><div align="left"><%=rs.getString("local")%></div></td>
          	<td><div align="left"><%=rs.getString("participante")%></div></td>
          	<td><div align="left"><%=rs.getString("aprovacao")%></div></td>
        </tr>
	<%
			}
		//Processo Paginação
	    String sqlConta = "select count(*) AS contaRegistros from atareuniao";
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_ata_reuniao.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_ata_reuniao.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_ata_reuniao.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_ata_reuniao.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <br>
	 <div align="center">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_ata_reuniao.jsp')"/>
       	<input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_ata_reuniao.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
 	</div>
</body>
</html>
