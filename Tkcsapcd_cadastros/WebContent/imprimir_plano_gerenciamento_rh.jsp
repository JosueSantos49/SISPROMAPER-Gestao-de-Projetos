...
<title>Imprimir Plano de Gerenciamento dos Recursos Humanos</title>
	...
	<table border="0" width="900" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h4>RELATÓRIO GERAL PLANO DE GERENCIAMENTO DOS RECURSOS HUMANOS</h4></font>	
								<div align="right">
...
<div class="table-responsive">
   <table class="table" id="tableID"> 
    <tr class="info">
    	<td ><div align="" class="style6"><a href="imprimir_plano_gerenciamento_rh.jsp?ordenacao=crescente">CÓDIGO</a></div></td>
      	<td ><div align="" class="style6"><a href="imprimir_plano_gerenciamento_rh.jsp?ordenacao=alfabetica">PROJETO</a></div></td>
      	<td ><div align="" class="style5"><strong>PLANO GER. rh</strong></div></td>  
	...
	try
	{
		...
		//Processo Paginação
				...
				String sql1 = "select * from planogerenciamentorh LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from planogerenciamentorh where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from planogerenciamentorh order by codigoprojeto");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from planogerenciamentorh");
			}		
		else if(request.getParameter("ordenacao").equals("alfabetica"))
			{
				rs = st.executeQuery("select * from planogerenciamentorh order by notarevisao");
			}
	...
	<tr class="default">
        ...
        <tr class="success">
         	<td><div align="left"><%=rs.getString("codigoprojeto")%></div></td>
          	...
          	<td><div align="left"><%=rs.getString("planogerenciamentorh")%></div></td>
	...
	<%
		...
		//Processo Paginação
	    String sqlConta = "select count(*) AS contaRegistros from planogerenciamentorh";
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_plano_gerenciamento_rh.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_plano_gerenciamento_rh.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_plano_gerenciamento_rh.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_plano_gerenciamento_rh.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <br>
	 <div align="center">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_plano_gerenciamento_rh.jsp')"/>
       	<input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_plano_gerenciamento_rh.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
 	</div>
</body>
</html>
