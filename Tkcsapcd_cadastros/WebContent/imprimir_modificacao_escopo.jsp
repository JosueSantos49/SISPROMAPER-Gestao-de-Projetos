...
<title>Imprimir Solicitação de Modificação de Escopo - ME</title>
	...
	<table border="0" width="900" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO GERAL DE SOLICITAÇÃO DE MODIFICAÇÃO DE ESCOPO - ME</h3></font>	
								<div align="right">
...
<div class="table-responsive">
   <table class="table" > 
    <tr class="info">
    	<td ><div ><a href="imprimir_modificacao_escopo.jsp?ordenacao=crescente">CÓDIGO</a></div></td>
      	<td ><div ><a href="imprimir_modificacao_escopo.jsp?ordenacao=alfabetica">EMPRESA</a></div></td>
      	<td ><div ><strong>ORGÃO</strong></div></td>
      	<td ><div ><strong>SETOR</strong></div></td>
      	<td ><div ><strong>PROGRAMA</strong></div></td>  
      	<td ><div ><strong>PROJETO</strong></div></td>
      	<td ><div ><strong>GERENTE</strong></div></td> 
      	<td ><div ><strong>REQUERIDO</strong></div></td>
      	<td ><div ><strong>ME N°</strong></div></td>
      	<td ><div ><strong>Data da emissão:</strong></div></td>   
    </tr>
    <%
    Connection con = null;
	...
	try
	{
		...
		String sql1 = "select * from modificacaoescopo LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from modificacaoescopo where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from modificacaoescopo order by codigoprojeto");
			}
			...
			{
				rs = st.executeQuery("select * from modificacaoescopo order by dataaprovacao");
			}
	...
	<tr class="default">
        ...
        <tr class="success">
         	<td><div align="left"><%=rs.getString("codigoprojeto")%></div></td>
          	...
          	<td><div align="left"><%=rs.getString("requerido")%></div></td>
          	<td><div align="left"><%=rs.getString("numero_me")%></div></td>
          	<td><div align="left"><%=rs.getString("dataemissao")%></div></td>
        </tr>
	<%
			}
		
		//Processo Paginação
	    String sqlConta = "select count(*) AS contaRegistros from modificacaoescopo";
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_modificacao_escopo.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_modificacao_escopo.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_modificacao_escopo.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_modificacao_escopo.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <br>
	 <div align="center">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_modificacao_escopo.jsp')"/>
       	<input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_modificacao_escopo.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
 	</div>
</body>
</html>
