<!-- 
	...
	Twproject Gantt é uma ferramenta online gratuita para criar Gantts. É um componente JavaScript construído sobre 
	jQuery para criar Gantts, árvores de tarefas, dependências que exporta os dados resultantes no formato JSON.
	Licenca MIT
 -->
<%@ page language="java" contentType="text/html; charset=ISO-8859-1" pageEncoding="ISO-8859-1"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<%@page import="java.util.List,br.com.tkcsapcd.model.bean.DiagramaGantt"%>

<html>
<head>
  <meta http-equiv="X-UA-Compatible" content="IE=9; IE=8; IE=7; IE=EDGE"/>
  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
  <title>Cadastrar Diagrama Gantt</title>
  	  
  <link rel=stylesheet href="gantt/platform.css" type="text/css">
  <link rel=stylesheet href="gantt/libs/dateField/jquery.dateField.css" type="text/css">
  ...
  <script src="gantt/libs/jquery.livequery.min.js"></script>
  <script src="gantt/libs/jquery.timers.js"></script>
  <script src="gantt/libs/platform.js"></script>
  <script src="gantt/libs/date.js"></script>
  <script src="gantt/libs/i18nJs.js"></script>
  <script src="gantt/libs/dateField/jquery.dateField.js"></script>
  <script src="gantt/libs/JST/jquery.JST.js"></script>

  <link rel="stylesheet" type="text/css" href="gantt/libs/jquery.svg.css">
  <script type="text/javascript" src="gantt/libs/jquery.svg.min.js"></script>

  <!--In case of jquery 1.7-->
  <!--<script type="text/javascript" src="gantt/libs/jquery.svgdom.pack.js"></script>-->

  <!--In case of jquery 1.8-->
  <script type="text/javascript" src="gantt/libs/jquery.svgdom.1.8.js"></script>


  <script src="gantt/ganttUtilities.js"></script>
  <script src="gantt/ganttTask.js"></script>
  <script src="gantt/ganttDrawerSVG.js"></script>
  <!--<script src="gantt/ganttDrawer.js"></script>-->
  <script src="gantt/ganttGridEditor.js"></script>
  <script src="gantt/ganttMaster.js"></script>  

 <script>
function myFunction() {
    document.getElementById("gimmeBack").submit();
}
</script>
...
<center><h5><font color="green"> ${mensagem}</font></h5></center>
	...
	<div id="ndo" style="position:absolute;right:5px;top:5px;width:378px;padding:5px;background-color: #FFF5E6; border:1px solid #F9A22F; font-size:12px" class="noprint">
	  <a href="index_portfolio_projeto.jsp" target="_parent"> Principal Portfólio</a>
	  <br><a href="index_principal.jsp" target="_parent">Principal Home</a>
	</div>
	
	
	<div id="workSpace" style="padding:0px; overflow-y:auto; overflow-x:hidden;border:1px solid #e5e5e5;position:relative;margin:0 5px"></div>
	
	<form id="gimmeBack" style="display:none;" action="TkcsapcdController" target="_parent">	
		
	<input type="hidden" name="cmd" value="cadastrarDiagramaGantt"></input>
	
		<div id="taZone" style="display:none;" class="noprint">
		   <textarea rows="8" cols="150" id="ta" name="ta">
		      {"tasks":[
		     {"id":-1,"name":"","code":"","level":0,"status":"STATUS_ACTIVE","canWrite":true,"start":1396994400000,"duration":0,"end":1399672799999,"startIsMilestone":true,"endIsMilestone":false,"collapsed":false,"assigs":[],"hasChild":true}
...
var ge;  //este é o var mundial feio, mas muito amigável para o editor de Gantt
$(function() {
  ...
  $(".ganttButtonBar div").append("<button type='submit' onclick='myFunction()' class='button'>Submit</button>");/*Submeter o formulário*/  
  $(".ganttButtonBar div").append("<button type='button' onclick='loadGanttFromServer();' class='button'>Load</button>");  
    
  $(".ganttButtonBar h1").html("<a href='#' title='' target='_parent'><img width='80%' src=''></a>");/*../gantt/res/twBanner.jpg Aqui estava o banner superior esquerdo.*/
  $(".ganttButtonBar div").addClass('buttons');
  
  //substituir por outras localizadas
  loadI18n();

  //simular um carregamento de dados a partir de um servidor.
  loadGanttFromServer();


  //preencher padrão Trabalho de papéis, se houver
  if (!ge.roles || ge.roles.length == 0) {
    setRoles();
  }

  //preencher recursos padrão papéis se houver
...
function loadGanttFromServer(taskId, callback) {
  ...
  //esta é uma simulação: carregar dados do armazenamento local se você já jogou com a demonstração de um textarea com o início de dados de demonstração
  ...
  $.getJSON("ganttAjaxController.jsp", {CM:"LOADPROJECT",taskId:taskId}, function(response) { //ganttAjaxController.jsp
    console.debug(response);
    if (response.ok) {
      prof.stop();

      ge.loadProject(response.project);
      ge.checkpoint(); //esvaziar a pilha de desfazer
...
function saveGanttOnServer() {
  ...
  //esta é uma simulação: salvar dados para o armazenamento local ou para o textarea
  saveInLocalStorage();
  ...
  $.ajax("TkcsapcdController?cmd=editarDiagramaGantt", { //ganttAjaxController.jsp
    dataType:"json",
    ...
    success: function(response) {
      if (response.ok) {
        prof.stop();
        if (response.project) {
          ge.loadProject(response.project); //deve recarregar como "tmp_" ids são agora os bons
        } else {
          ge.reset();
        }
      } else {
...
//-------------------------------------------  Obter arquivo de projeto como JSON (usado para o projeto de migração de Gantt para trabalhos de equipa) ------------------------------------------------------
function getFile() {
...
//-------------------------------------------  LOCAL gerenciamento de armazenamento (por esta demonstração apenas)------------------------------------------------------
...
//Abra um pop-up preto para gerir os recursos. Este é apenas um exemplo de aplicação (geralmente recursos vêm de servidor)
