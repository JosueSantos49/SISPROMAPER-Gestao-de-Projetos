...
<title>Imprimir Stakeholders</title>
	...
	<table border="0" width="900" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO GERAL DE STAKEHOLDERS</h3></font>	
								<div align="right">
...
<div class="table-responsive">
  <table class="table">
    <tr class="info">
    	<td ><a href="imprimir_stakeholders.jsp?ordenacao=crescente">CÓDIGO</a></div></td>
      	<td ><a href="imprimir_stakeholders.jsp?ordenacao=alfabetica">EMPRESA</a></div></td>
      	<td ><strong>ORGÃO</strong></div></td>
      	<td ><strong>SETOR</strong></div></td>
      	<td ><strong>PROGRAMA</strong></div></td>  
      	<td ><strong>PROJETO</strong></div></td>
      	<td ><strong>GERENTE</strong></div></td> 
      	<td ><strong>ELABORADO</strong></div></td>
      	<td ><strong>APROVADO</strong></div></td>
      	<td ><strong>VERSÃO</strong></div></td>
      	<td ><strong>DATA APROVAÇÃO</strong></div></td>     
    </tr>
    <%
    Connection con = null;
	...
	try
	{
		...
		//Processo Paginação
				...
				String sql1 = "select * from stakeholders LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from stakeholders where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from stakeholders order by codigoprojeto");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from stakeholders");
			}		
	...
	<%
		...
		//Processo Paginação
	    String sqlConta = "select count(*) AS contaRegistros from stakeholders";
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_stakeholders.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_stakeholders.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_stakeholders.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_stakeholders.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <br>
	 <div align="center">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_stakeholders.jsp')"/>
       	<input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_stakeholders.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
 	</div>
</body>
</html>
