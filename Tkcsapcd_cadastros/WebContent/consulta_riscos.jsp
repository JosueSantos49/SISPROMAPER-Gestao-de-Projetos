...
<%@page import="br.com.tkcsapcd.model.bean.Riscos"%>
...
<title>Consultar Gerenciamento Riscos</title>
...
<body onload="waitingDialog.show();setTimeout(function () {waitingDialog.hide();}, 2000); tamanhooriginal(); setFocus();">
	...
	<div id="conteudo">
		...
		<div align="justify">
			...
			<div align="center">
				<p>
					<font color="red">Consultar Gerenciamento Riscos</font>
				</p>
			</div>
			<br>
			...
			<div class="table-responsive">
				<table id="tabela" cellspacing="5" cellspacing="5" class="display">
					...
					<tbody>
							<%
								List<Riscos> riscoss = (List<Riscos>) request.getAttribute("riscoss");
								for (Riscos riscos : riscoss) {
							%>
						<tr>							
							<td><%=riscos.getProjetoinicio().getCodigo()%></td>
							<td><%=riscos.getEmpresa()%></td>
							<td><%=riscos.getOrgao()%></td>
							<td><%=riscos.getSetor()%></td>
							<td><%=riscos.getPrograma()%></td>
							<td><%=riscos.getGerente()%></td>
							<td><%=riscos.getFuncao()%></td>
							<td><%=riscos.getAprovado()%></td>
							<td><%=riscos.getVersao()%></td>							
							<td><%=riscos.getDataaprovacao()%></td>
							<td><a class="link_atualizar"
								href='TkcsapcdController?cmd=editarRiscos&codigo=<%=riscos.getCodigo()%>'><img
									alt="" title="Atualizar" height="20" width="27"
									src="img/alterar.gif"></a></td>
							<td><a class="link_delete"
								href='TkcsapcdController?cmd=excluirRiscos&codigo=<%=riscos.getCodigo()%>'><img
