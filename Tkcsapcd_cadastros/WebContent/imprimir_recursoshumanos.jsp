...
<title>Relatório Recursos Humanos</title>
...
<table border="0" width="800" align="center" >
		<tr>			
			<td width="350" height="45px" colspan="2" align="center">
				...
				<h3><font color="#0404B4">RELATÓRIO GERAL RECURSOS HUMANOS</font></h3>	
								<div align="right">
...
<div class="table-responsive">
   <table class="table" id="tableID"> 
    <tr class="info">
      <td><div align="center" class="style6"><a href="imprimir_recursoshumanos.jsp?ordenacao=crescente">ID Recursos Humanos</a></div></td>
      <td><div align="center" class="style5"><strong><a href="imprimir_recursoshumanos.jsp?ordenacao=alfabetica">Nome</a></strong></div></td>
	...
	{		
		...
		//Processo Paginação
				...
				String sql1 = "select * from recursoshumanos LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from recursoshumanos where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
			rs = st.executeQuery("select * from recursoshumanos order by nome");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from recursoshumanos");
			}		
	...
	<tr class="default">
        ...
        <tr class="success">                
          <td ><div ><%=rs.getString("idRecursosHumanos")%></div></td>
          <td ><div ><%=rs.getString("nome")%></div></td>
          <td><div ><%=rs.getString("profissao")%></div></td>
	...
	<%
		...
		//Processo Paginação
	    String sqlConta = "select count(*) AS contaRegistros from recursoshumanos";
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_recursoshumanos.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_recursoshumanos.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_recursoshumanos.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_recursoshumanos.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <table width="400" border="0">
    <tr>
      <td><div align="center"> <span class="style1 style2 style5">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_recursoshumanos.jsp')"/>
        <input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_recursoshumanos.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
	  </span></div></td>
    </tr>
  </table>
</div>
