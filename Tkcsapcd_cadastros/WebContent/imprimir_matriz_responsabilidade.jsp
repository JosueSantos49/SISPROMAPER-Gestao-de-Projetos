...
<title>Imprimir Matriz de Responsabilidade</title>
	...
	<table border="0" width="900" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO MATRIZ DE RESPONSABILIDADE</h3></font>	
								<div align="right">
...
<div class="table-responsive">
   <table class="table" > 
    <tr class="info">
    	<td width="90"><div align="" class="style6"><a href="imprimir_matriz_responsabilidade.jsp?ordenacao=crescente">CÓDIGO EAP</a></div></td>
      	<td width="90"><div align="" class="style6"><a href="imprimir_matriz_responsabilidade.jsp?ordenacao=alfabetica">ATIVIDADE</a></div></td>
      	<td width="25"><div align="" class="style5"><strong>RESPONSÁVEL</strong></div></td>
      	<td width="25"><div align="" class="style5"><strong>APROVAR</strong></div></td>
      	<td width="90"><div align="" class="style5"><strong>CONSULTADO</strong></div></td>
      	<td width="90"><div align="" class="style5"><strong>INFORMADO</strong></div></td>    
    </tr>
    <%
    Connection con = null;
	...
	try
	{
		...
		String sql1 = "select * from matrizresponsabilidade LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from matrizresponsabilidade where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from matrizresponsabilidade order by codigoeap");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from matrizresponsabilidade");
			}		
		else if(request.getParameter("ordenacao").equals("alfabetica"))
			{
				rs = st.executeQuery("select * from matrizresponsabilidade order by atividade");
			}
    ...
    <tr class="default">		
        ...
        <tr class="success">
         	<td><div align="left"><%=rs.getString("codigoeap")%></div></td>
          	<td><div align="left"><%=rs.getString("atividade")%></div></td>          
          	<td><div align="left"><%=rs.getString("responsavel")%></div></td>
          	<td><div align="left"><%=rs.getString("aprovar")%></div></td>
          	<td><div align="left"><%=rs.getString("consultado")%></div></td>
          	<td><div align="left"><%=rs.getString("informado")%></div></td>
        </tr>
	<%
			}
		
		//Processo Paginação
	    String sqlConta = "select count(*) AS contaRegistros from matrizresponsabilidade";
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_matriz_responsabilidade.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_matriz_responsabilidade.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_matriz_responsabilidade.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_matriz_responsabilidade.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <br>
	 <div align="center">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_matriz_responsabilidade.jsp')"/>
       	<input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_matriz_responsabilidade.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
 	</div>
</body>
</html>
