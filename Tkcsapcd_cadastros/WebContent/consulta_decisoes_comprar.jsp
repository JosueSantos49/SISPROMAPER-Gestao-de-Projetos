...
<title>Consultar Decisões de Comprar</title>
...
<body onload="waitingDialog.show();setTimeout(function () {waitingDialog.hide();}, 2000); tamanhooriginal(); setFocus();">
	...
	<div id="conteudo">
		...
		<div align="justify">
			...
			<div align="center">
				<p>
					<font color="red">Consultar Decisões de Comprar</font>
				</p>
			</div>
			<br>
			...
			<div align="left">
				...
				<form id="formConsProjeto" name="formConsProjeto" method="post"
					action="pesquisar_decisoes_comprar.jsp"
					...
					<strong>Relatório..:</strong> <input name="consProjeto" type="text"
						...
						required="true" Placeholder="Código EAP!"> <input
			...
			<div class="table-responsive">
				<table id="tabela" cellspacing="5" cellspacing="5" class="display">
					...
					<tbody>
							<%
								List<DecisoesComprar> decisoesComprars = (List<DecisoesComprar>) request.getAttribute("decisoesComprars");
								for (DecisoesComprar decisoesComprar : decisoesComprars) {
							%>
						<tr>
							<td><%=decisoesComprar.getCodigoeap()%></td>
							<td><%=decisoesComprar.getImportancia()%></td>
							<td><%=decisoesComprar.getUrgencia()%></td>
							<td><%=decisoesComprar.getImpacto()%></td>
							<td><%=decisoesComprar.getQuantidade()%></td>
							<td><%=decisoesComprar.getCusto()%></td>
							<td><%=decisoesComprar.getStatus()%></td>
							<td><a class="link_atualizar"
								href='TkcsapcdController?cmd=editarDecisoesComprar&codigo=<%=decisoesComprar.getCodigo()%>'><img
									alt="" title="Atualizar" height="20" width="27"
									src="img/alterar.gif"></a></td>
							<td><a class="link_delete"
								href='TkcsapcdController?cmd=excluirDecisoesComprar&codigo=<%=decisoesComprar.getCodigo()%>'><img
