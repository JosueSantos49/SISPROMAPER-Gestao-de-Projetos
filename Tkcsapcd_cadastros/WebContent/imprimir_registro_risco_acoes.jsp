...
<title>Imprimir Registro dos Riscos Ações</title>
	...
	<table border="0" width="900" align="center" >
		<tr>			
			<td width="350" height="70" align="center">
				...
				<font color="#0404B4"><h3>RELATÓRIO GERAL REGISTRO DOS RISCOS AÇÕES</h3></font>	
								<div align="right">
...
<div class="table-responsive">
   <table class="table" id="tableID"> 
    <tr class="info">
    	<td ><div align="" class="style6"><a href="imprimir_registro_risco_acoes.jsp?ordenacao=crescente">CÓDIGO</a></div></td>
      	<td ><div align="" class="style6"><a href="imprimir_registro_risco_acoes.jsp?ordenacao=alfabetica">PROJETO</a></div></td>
      	<td ><div align="" class="style5"><strong>REGI. RISCO AÇÕES</strong></div></td>  
	...
	try
	{
		...
		String sql1 = "select * from registroriscoacoes LIMIT 30 OFFSET 0"+offset;
		...
		if(request.getParameter("acao") != null)
		{
			...
			st.executeUpdate("delete from registroriscoacoes where codigo = "+request.getParameter("codigo"));
		}
		...
		if(request.getParameter("ordenacao") == null)
			{
				rs = st.executeQuery("select * from registroriscoacoes order by codigoprojeto");
			}
		else if(request.getParameter("ordenacao").equals("crescente"))
			{
				rs = st.executeQuery("select * from registroriscoacoes");
			}		
		else if(request.getParameter("ordenacao").equals("alfabeticaacoes"))
			{
				rs = st.executeQuery("select * from registrorisco order by notarevisao");
			}
	...
	<tr class="default">
        ...
        <tr class="success">
         	<td><div align="left"><%=rs.getString("codigoprojeto")%></div></td>
          	...
          	<td><div align="left"><%=rs.getString("registroriscoacoes")%></div></td>
	...
	<%
		...
		//Processo Paginação
	    String sqlConta = "select count(*) AS contaRegistros from registroriscoacoes";
		...
		if(Integer.parseInt(numPagina) > 1){
			...
			out.println("<li><a href=imprimir_registro_risco_acoes.jsp?numpagina="+pagAnterior+"> << Anterior </a></li>");
			out.println("</ul>");
		}			
		for(int i=1;i<=totalPaginas;i++){
			if(i == Integer.parseInt(numPagina))
				...
				out.println("<a href=imprimir_registro_risco_acoes.jsp?numpagina="+i+"></a>");
				out.println("<ul class='pagination'>");
				out.println("<li><a href=imprimir_registro_risco_acoes.jsp?numpagina="+i+">"+ i +"</a></li>");
				out.println("</ul>");
		}											
		int proximaPag;
		if(totalRegistros - (Integer.parseInt(numPagina) * limite) > 0){
			...
			out.println("<ul class='pagination'>");
			out.println("<li><a href=imprimir_registro_risco_acoes.jsp?numpagina="+proximaPag+"> Próxima >> </a></li>");
			out.println("</ul>");
		}
  ...
  <br>
	 <div align="center">
        ...
        <input type="button" name="imprimir" id="imprimir" value="Impressão em tela" onclick="window.open('imprimir_registro_risco_acoes.jsp')"/>
       	<input type="button" name="imprimir" id="imprimir" value="Relatório pop up" onclick="window.open('imprimir_registro_risco_acoes.jsp','relatorio','toolbar=0,scrollbars=1,location=0,directories=0,copyhistory=0,status=0,menubar=0,resizable=0,width=800,height=600,z-lock,screenX=90,screenY=100,Left=100,Top=20')"/>
 	</div>
</body>
</html>
